...
describe('WeeklyRefreshPanel', () => {
  it('reports the current board as loaded when live and online', () => {
    const { getByText, getByRole } = renderWithProviders(() => (
      <WeeklyRefreshPanel state={LIVE_STATE} onRefresh={() => {}} />
    ))
  ...
  it('disables refresh and reports being offline when there is no connection', () => {
    const { getByText, getByRole } = renderWithProviders(() => (
      <WeeklyRefreshPanel state={{ ...LIVE_STATE, online: false, source: 'cache' }} onRefresh={() => {}} />
    ))
  ...
  it('warns that only the last known board is shown once the cycle has expired', () => {
    const { getByText } = renderWithProviders(() => (
      <WeeklyRefreshPanel state={{ ...LIVE_STATE, expired: true }} onRefresh={() => {}} />
    ))
  ...
  it('reports an in-progress refresh', () => {
    const { getByText, getByRole } = renderWithProviders(() => (
      <WeeklyRefreshPanel state={{ ...LIVE_STATE, refreshing: true }} onRefresh={() => {}} />
    ))
  ...
  it('fires onRefresh when the refresh button is clicked', () => {
    ...
    const { getByRole } = renderWithProviders(() => <WeeklyRefreshPanel state={LIVE_STATE} onRefresh={onRefresh} />)
  ...
  it('flashes the button success once a refresh settles without a failure', () => {
    const { getByRole } = renderWithProviders(() => <WeeklyRefreshPanel state={LIVE_STATE} onRefresh={() => {}} />)
  ...
  it('flashes the button danger when the board reports a failed refresh', () => {
    const { getByRole } = renderWithProviders(() => (
      <WeeklyRefreshPanel state={{ ...LIVE_STATE, refreshFailed: true }} onRefresh={() => {}} />
    ))
    const button = getByRole('button', { name: 'Refresh' })
