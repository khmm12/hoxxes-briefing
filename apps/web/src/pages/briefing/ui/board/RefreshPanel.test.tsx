...
describe('RefreshPanel', () => {
  it('reports the current briefing as loaded when live and online', () => {
    const { getByText, getByRole } = renderWithProviders(() => <RefreshPanel state={LIVE_STATE} onRefresh={() => {}} />)

    expect(getByText('Current briefing loaded.')).toBeInTheDocument()
  ...
  it('disables refresh and reports being offline when there is no connection', () => {
    ...
    expect(getByText("Saved briefing loaded. You're offline for now.")).toBeInTheDocument()
    expect(getByRole('button', { name: 'Offline' })).toBeDisabled()
  })

  it('warns that only the last known briefing is shown once the cycle has expired', () => {
    const { getByText } = renderWithProviders(() => (
      <RefreshPanel state={{ ...LIVE_STATE, expired: true }} onRefresh={() => {}} />
    ))

    expect(getByText('Last known briefing only. This cycle already ended.')).toBeInTheDocument()
  })
  ...
  it('reports an in-progress refresh', () => {
    ...
    expect(getByText('Refreshing current briefing now.')).toBeInTheDocument()
  ...
  it('flashes the button danger when the briefing reports a failed refresh', () => {
