...
describe('BriefingLoadingState', () => {
  it('tells the user it is pulling fresh dives when online', () => {
    const { getByText } = renderWithProviders(() => <BriefingLoadingState dockVisible={false} online={true} />)
  ...
  it('points at the on-device lookup when offline', () => {
    const { getByText } = renderWithProviders(() => <BriefingLoadingState dockVisible={false} online={false} />)
